...
import './Section1.css'; // Use your existing styles
...
const Section3 = () => {
  ...
  const [sectionTitle, setSectionTitle] = useState('Section 3: Advanced Evaluation');
    ...
    const fetchQuestions = async () => {
      ...
      try {
        ...
        if (data.sections && data.sections.length > 2) {
          setSectionTitle(data.sections[2].title || 'Section 3: Advanced Evaluation');

          const randomShowstoppers = new Set();
          while (
            randomShowstoppers.size < 5 &&
            randomShowstoppers.size < data.sections[2].questions.length
          ) {
            randomShowstoppers.add(Math.floor(Math.random() * data.sections[2].questions.length));
          }

          const initializedQuestions = data.sections[2].questions.map((q, i) => ({
            ...q,
            comments: q.comments || '',
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/projects/${projectId}/crrs/${crrId}/section/3`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        navigate(`/projects/${projectId}/crrs/${crrId}/section4`);
      } else {
    ...
    <div className="crr-section">
      ...
      <div className="questions-section">
        {questions.length > 0 ? (
          ...
          <p style={{ color: 'red', fontWeight: 'bold' }}>⚠️ Aucune question trouvée pour la Section 3.</p>
        )}
      </div>
