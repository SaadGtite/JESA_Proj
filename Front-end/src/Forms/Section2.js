...
import './Section1.css'; // You can rename this if needed
...
const Section2 = () => {
  ...
  const [sectionTitle, setSectionTitle] = useState('Section 2: Advanced Evaluation');
    ...
    const fetchQuestions = async () => {
      ...
      try {
        ...
        if (data.sections && data.sections.length > 1) {
          setSectionTitle(data.sections[1].title || 'Section 2: Advanced Evaluation');

          const randomShowstoppers = new Set();
          while (
            randomShowstoppers.size < 5 &&
            randomShowstoppers.size < data.sections[1].questions.length
          ) {
            randomShowstoppers.add(Math.floor(Math.random() * data.sections[1].questions.length));
          }

          const initializedQuestions = data.sections[1].questions.map((q, i) => ({
            ...q,
            comments: q.comments || '',
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/projects/${projectId}/crrs/${crrId}/section/2`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        navigate(`/projects/${projectId}/crrs/${crrId}/section3`);
      } else {
    ...
    <div className="crr-section">
      ...
      <div className="questions-section">
        {questions.length > 0 ? (
          ...
          <p style={{ color: 'red', fontWeight: 'bold' }}>⚠️ Aucune question trouvée pour la Section 2.</p>
        )}
      </div>
